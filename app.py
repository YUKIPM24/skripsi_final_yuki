...
def predict_text_page():
    ...
    if submit:
        ...
        tweet = convert_slangword(tweet)
        tweet = remove_stopwords_with_exception(tweet)
        tweet = RemoveUnwantedwords(tweet)
        tweet = ' '.join(re.findall(r'\w{3,}', tweet))
        
        text_tfidf = tfidf.transform([tweet])
...
def predict_dataframe_page():
    ...
    if uploaded_file:
        ...
        if submit_df:
            ...
            # Assuming you want to use the lexicon sentiment labels for the word clouds
            positive_text = ' '.join(result_df[result_df['sentiment'] == 'positif']["Text_Clean_lagi"])
            negative_text = ' '.join(result_df[result_df['sentiment'] == 'negatif']["Text_Clean_lagi"])
            all_text = ' '.join(result_df["Text_Clean_lagi"])
            ...
            with pos_col1:
                ...
                # positive = result_df[result_df['sentiment'] == 'positif']
                # # Tokenisasi kalimat positif
                # all_words = []
                # for sentence in positive["Text_Clean_lagi"]: 
                ...
                # # Buat DataFrame dari top 10 kata
                # top_10_df = pd.DataFrame(top_10_words, columns=['Positif Word', 'Frequency'])
                st.dataframe(result3)
                
            with neg_col2:
                st.subheader("Top 10 Negative Word")
                # negitive = result_df[result_df['sentiment'] == 'negatif']
                # # Tokenisasi kalimat positif
                # all_words = []
                # for sentence in negitive["Text_Clean_lagi"]: 
                #     tokens = word_tokenize(sentence.lower())
                #     all_words.extend(tokens)
                ...
                # # Buat DataFrame dari top 10 kata
                # top_10_df = pd.DataFrame(top_10_words, columns=['Negatif Word', 'Frequency'])
